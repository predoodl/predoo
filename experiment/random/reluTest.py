...
def torch_reluWithDiffType(dtype):
    torch_relu = torch.nn.ReLU()
    torch_relu.type(dtype)
    return torch_relu
...
def getdataforTorch(f):
    ...
    for j in range(1000):
        ...
        for i in range(100):
            ...
            torch_relu_16 = torch_reluWithDiffType(torch.float16)
            torch_relu_32 = torch_reluWithDiffType(torch.float32)
            torch_relu_64 = torch_reluWithDiffType(torch.float64)

            out_32_32_1 = torch_relu_32(x_32).detach().numpy()
            out_32_16 = torch_relu_16(x_32).detach().numpy()
            out_32_64 = torch_relu_64(x_32).detach().numpy()
            ...
            out_64_16 = torch_relu_16(x_64).detach().numpy()
            out_64_32 = torch_relu_32(x_64).detach().numpy()
            out_64_64 = torch_relu_64(x_64).detach().numpy()
...
def getDataforTensorflow(f):
    ...
    for j in range(1000):
        ...
        for i in range(100):
            ...
            tf_relu_16 = tf_reluWithDiffDype('float16')
            tf_relu_32 = tf_reluWithDiffDype('float32')
            tf_relu_64 = tf_reluWithDiffDype('float64')

            out_16_16_1 = tf_relu_16(x_16).numpy().astype(np.float32)
            out_16_16_2 = tf_relu_16(x_16).numpy().astype(np.float64)
            out_16_32 = tf_relu_32(x_16)
            out_16_64 = tf_relu_64(x_16)
            ...
            out_32_32_1 = tf_relu_32(x_32)
            out_32_32_2 = tf_relu_32(x_32).numpy().astype(np.float64)
            out_32_16 = tf_relu_16(x_32).numpy().astype(np.float32)
            out_32_64 = tf_relu_64(x_32)
            ...
            out_64_16 = tf_relu_16(x_64).numpy().astype(np.float64)
            out_64_32 = tf_relu_32(x_64).numpy().astype(np.float64)
            out_64_64 = tf_relu_64(x_64)
...
def getDataforTfWihthG(f,g):
    ...
    for i in range(20):
        ...
        for j in range(1000):
            ...
            s = time.time()
            out_16_16 = tf_relu_16(x_16)
            diff7 = np.mean(np.abs(tf_relu_16(x_32) - out_16_16))
            diff8 = np.mean(np.abs(tf_relu_16(x_64) - out_16_16))

            diff9 = np.mean(np.abs(tf_relu_32(x_16) - out_32_32_1))
            diff10 = np.mean(np.abs(tf_relu_32(x_64) - out_32_32_1))

            diff11 = np.mean(np.abs(tf_relu_64(x_16) - out_64_64))
            diff12 = np.mean(np.abs(tf_relu_64(x_32) - out_64_64))
...
if __name__=='__main__':
    getDataforTfWihthG("/home/ise/opTest/data/timeflow2/tf_gpu_2.3.1/relu.csv",
                       "/home/ise/opTest/data/timeflow2/tf_gpu_2.3.1/relu_count.csv")
