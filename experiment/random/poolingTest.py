...
def torch_poolingWithDiffType(dtype):
    torch_pooling = torch.nn.MaxPool2d(kernel_size=2, stride=1)
    torch_pooling.type(dtype)
    return torch_pooling


def getdataforTorch(f):     #torch_cpu_1.7.0
    ...
    for j in range(1000):
        ...
        for i in range(100):
            ...
            torch_pooling_16 = torch_poolingWithDiffType(torch.float16)
            torch_pooling_32 = torch_poolingWithDiffType(torch.float32)
            torch_pooling_64 = torch_poolingWithDiffType(torch.float64)

            out_32_32_1 = torch_pooling_32(x_32).detach().numpy()
            out_32_16 = torch_pooling_16(x_32).detach().numpy()
            out_32_64 = torch_pooling_64(x_32).detach().numpy()
            ...
            out_64_16 = torch_pooling_16(x_64).detach().numpy()
            out_64_32 = torch_pooling_32(x_64).detach().numpy()
            out_64_64 = torch_pooling_64(x_64).detach().numpy()
...
def getDataforTfWihthG(f,g):
    ...
    for i in range(20):
        ...
        for j in range(1000):
            ...
            diff7 = np.mean(np.abs(tf_pooling_16(x_32) - out_16_16))
            diff8 = np.mean(np.abs(tf_pooling_16(x_64) - out_16_16))

            diff9 = np.mean(np.abs(tf_pooling_32(x_16) - out_32_32_1))
            diff10 = np.mean(np.abs(tf_pooling_32(x_64) - out_32_32_1))

            diff11 = np.mean(np.abs(tf_pooling_64(x_16) - out_64_64))
            diff12 = np.mean(np.abs(tf_pooling_64(x_32) - out_64_64))
...
if __name__=='__main__':
    getDataforTfWihthG("/home/ise/opTest/data/timeflow2/tf_gpu_2.3.1/pooling.csv",
                      "/home/ise/opTest/data/timeflow2/tf_gpu_2.3.1/pooling_count.csv")
